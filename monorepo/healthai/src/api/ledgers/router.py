...
healthai_router = APIRouter()


def get_ledger_service(db: Session = Depends(get_db)) -> BaseLedgerService:
    return BaseLedgerService(LedgerRepository(db=db, model=HealthAILedgerEntryModel))


@healthai_router.get("/ledger/{owner_id}")
def get_balance(
...
def add_ledger_entry(
    ...
    if balance < amount and operation != HealthAILedgerOperation.CREDIT_ADD:
